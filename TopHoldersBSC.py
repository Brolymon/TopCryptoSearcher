...
def parse_token_page(token_url,holders_url):

    token_page = get_token_page(token_url)
        ...
        if overview_element is not None:
            ...
            if overview_dictionary["token_holders"] != 0:
                parsed_body = BeautifulSoup(
                    get_token_page(holders_url), "html.parser"
                )
...
holders_dict = parse_token_page("https://bscscan.com/token/0x308d6B4bDeD91f99b58bdaAfCC1137Fb4cFd4AB2#balances#balances",'https://bscscan.com/token/tokenholderchart/0x308d6B4bDeD91f99b58bdaAfCC1137Fb4cFd4AB2#balances?range=500')['holders_dictionary']

# for holder,data in holders_dict.items():
#      print(holder,data)
# from bscscan import BscScan
count = 0
...
address_list = []
# while ( i < 100):
#     for a in holders_dict[i]:
#         print(a)
#     i = i + 1

while ( i < 100):
    address_list.append({'address' : holders_dict[i]['address'],'percentage' : holders_dict[i]['percentage'] })
    i = i + 1
address_list1 = sorted(address_list, key=lambda d: d['percentage']) 
...
async def main():
  ...
  async with BscScan(YOUR_API_KEY) as client:
      while i < len(holders_dict):
        if(holders_dict[i]['address'][0] == '0'):
            rows = await client.get_bep20_token_transfer_events_by_address_and_contract_paginated(
                    contract_address= "0x308d6B4bDeD91f99b58bdaAfCC1137Fb4cFd4AB2",
                    address= holders_dict[i]['address'],
                    page= 1,
                    offset= 100,
                    sort= "asc"
                )
            newlist = sorted(rows, key=lambda d: d['timeStamp'],reverse = True) 
            for ab in newlist:
                if ab['from'] == holders_dict[i]['address']:
                    for d in address_list1:
                        if d['address'] == ab['to']:
                            dict_temp = {'address':d['address'],'percentage': d['percentage'], 'timeStamp': datetime.fromtimestamp(int(ab['timeStamp'])) }
                            final_dict[holders_dict[i]['address']].append(dict_temp)


        sleep(.1)
        i = i + 1
      for key,value in final_dict.items():
          print(f"address sending to top 100 = {key}")
          print("addresses recieved values")
          for row in value:
              print(row)
          print("-----------------------------------")
